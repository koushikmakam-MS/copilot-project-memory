...
7. **Auto-export to editors** — if `auto_export_editors: true` is set in `preferences.yml`, regenerate `.github/copilot-instructions.md` in the project root whenever rules, preferences, or context are updated. This keeps VS Code, JetBrains, and other editors in sync with project memory automatically. Only export shareable content (rules with `share: true` or without a `share` field, preferences, context). Do NOT export personal session history, interaction style, or error tracking.
...
When `auto_export_editors: true` is set in the project's `preferences.yml`, **automatically regenerate** `.github/copilot-instructions.md` whenever any of these events occur:
