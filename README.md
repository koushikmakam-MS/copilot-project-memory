...
/memory rules add-do "Use Zod for validation"
/memory rules add-dont "Never use any type"
/memory context         — Show project context
/memory context stack next.js
/memory extensions add dbaeumer.vscode-eslint ESLint
/memory stats           — Stats across ALL projects
```

#### Snippets
```
/memory snippets save api-handler    — Save last code block as snippet
/memory snippets get api-handler     — Retrieve a snippet
/memory snippets list                — List all snippets
```
...
```
This exports your project's rules and context (not personal history) into a file the whole team benefits from.
...
```
Useful for moving to a new machine beyond the initial installer.
...
```
/memory rules add-do --global "I prefer concise answers"
/memory rules add-dont --global "Never include unnecessary comments"
```
